...
class AutoAttention(nn.Module):
    def __init__(self, dim, num_patches, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,
                 sr_ratio=1, kernel_size=3, atten_cfg=None,  **kwargs):
        super().__init__()
        assert dim % num_heads == 0, f"dim {dim} should be divided by num_heads {num_heads}."
        ...
        self.dwc = nn.Conv2d(in_channels=dim, out_channels=dim, kernel_size=kernel_size, padding=1, groups=dim)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_patches, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 agent_num=49, downstream_agent_shape=(7, 7), kernel_size=3, attn_type='B', scale=-0.5, atten_cfg=None):
        super().__init__()
        self.norm1 = norm_layer(dim)
        assert attn_type in ['A', 'B']
        if attn_type == 'A':
            self.attn = AutoAttention(
                dim, num_patches,
                num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
                attn_drop=attn_drop, proj_drop=drop, sr_ratio=sr_ratio,
                agent_num=agent_num, downstream_agent_shape=downstream_agent_shape, kernel_size=kernel_size,
                scale=scale, atten_cfg=atten_cfg)
        else:
            self.attn = Attention(
                dim,
                num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
...
class AutoAtten_PVT(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 ...
                 kernel_size=3, attn_type='BBBB', atten_cfg=None, scale=-0.5, init_cfg=None, **kwargs):
        super().__init__()
        ...
        for i in range(num_stages):
            ...
            block = nn.ModuleList([Block(
                ...
                kernel_size=kernel_size, attn_type=attn_type[i], scale=scale, atten_cfg=atten_cfg)
    ...
    def train(self, mode=True):
        ...
        super(AutoAtten_PVT, self).train(mode)
