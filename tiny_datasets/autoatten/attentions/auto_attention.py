...
class AutoAttention_swin(nn.Module):
    ...
    def __init__(self, atten_cfg, window_size,
        ...
        if atten_cfg['bias_index'] and len(atten_cfg['bias_index']) == 1:

            self.na_bias = nn.Parameter(torch.zeros(num_heads, head_dim, 7, 7))
            self.ha_bias = nn.Parameter(torch.zeros(1, num_heads, window_size[0], 1, head_dim))
            self.wa_bias = nn.Parameter(torch.zeros(1, num_heads, 1, window_size[1], head_dim))
        ...
        elif atten_cfg['bias_index'] and len(atten_cfg['bias_index']) == 2:

            self.na_bias = nn.Parameter(torch.zeros(num_heads, head_dim, 7, 7))
            self.ha_bias = nn.Parameter(torch.zeros(1, num_heads, window_size[0], 1, head_dim))
            self.wa_bias = nn.Parameter(torch.zeros(1, num_heads, 1, window_size[1], head_dim))

            self.nb_bias = nn.Parameter(torch.zeros(num_heads, head_dim, 7, 7))
            self.hb_bias = nn.Parameter(torch.zeros(1, num_heads, window_size[0], 1, head_dim))
            self.wb_bias = nn.Parameter(torch.zeros(1, num_heads, 1, window_size[1], head_dim))

            trunc_normal_(self.na_bias, std=.02)
            trunc_normal_(self.ha_bias, std=.02)
            trunc_normal_(self.wa_bias, std=.02)

            trunc_normal_(self.nb_bias, std=.02)
            trunc_normal_(self.hb_bias, std=.02)
            trunc_normal_(self.wb_bias, std=.02)
    ...
    def forward(self, x, mask=None):
        ...
        if self.atten_cfg['bias_index'] and len(self.atten_cfg['bias_index']) == 1:
            bias1 = nn.functional.interpolate(self.na_bias, size= self.window_size, mode='bilinear')
            bias1 = bias1.reshape(1, num_heads, head_dim, -1).permute(0, 1, 3, 2).repeat(N, 1, 1, 1)

            bias2 = (self.ha_bias + self.wa_bias).reshape(1, num_heads, -1, head_dim).repeat(N, 1, 1, 1)
            auto_bias = bias1 + bias2

            diag_inputs = ([q,k,v], auto_bias)

        elif self.atten_cfg['bias_index'] and len(self.atten_cfg['bias_index']) == 2:
            bias1 = nn.functional.interpolate(self.na_bias, size=self.window_size, mode='bilinear')
            bias1 = bias1.reshape(1, num_heads, head_dim, -1).permute(0, 1, 3, 2).repeat(N, 1, 1, 1)
            bias2 = (self.ha_bias + self.wa_bias).reshape(1, num_heads, -1, head_dim).repeat(N, 1, 1, 1)
            auto_bias1 = bias1 + bias2

            bias3 = nn.functional.interpolate(self.nb_bias, size=self.window_size, mode='bilinear')
            bias3 = bias3.reshape(1, num_heads, head_dim, -1).permute(0, 1, 3, 2).repeat(N, 1, 1, 1)
            bias4 = (self.hb_bias + self.wb_bias).reshape(1, num_heads, -1, head_dim).repeat(N, 1, 1, 1)
            auto_bias2 = bias3 + bias4

            diag_inputs = ([q, k, v], auto_bias1, auto_bias2)
