...
class Auto_Attention(nn.Module):
    ...
    def __init__(self, atten_cfg, num_patches, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        super().__init__()
    ...
    def forward(self, x, H, W):
        ...
        v_ = v.transpose(1, 2).reshape(B, H // self.sr_ratio, W // self.sr_ratio, C).permute(0, 3, 1, 2)
        #
        if self.sr_ratio > 1:
            v_ = nn.functional.interpolate(v_, size=(H, W), mode='bilinear')
        out = out + self.dwc(v_).permute(0, 2, 3, 1).reshape(B, N, C)
...
class Block(nn.Module):
    ...
    def __init__(self, atten_cfg, num_patches, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, sr_ratio=1):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Auto_Attention(atten_cfg, num_patches,
            dim,
...
class PVT(BaseTransformerModel):
    ...
    def __init__(self, atten_config = None, num_classes = None):
        ...
        for i in range(self.num_stages):
            ...
            num_patches = patch_embed.num_patches \
                if i != self.num_stages - 1 else patch_embed.num_patches + 1
            pos_embed = nn.Parameter(torch.zeros(1, num_patches, self.hidden_dim[i]))
            pos_drop = nn.Dropout(p=self.drop_rate)

            block = nn.ModuleList([Block(self.atten_config, num_patches,
