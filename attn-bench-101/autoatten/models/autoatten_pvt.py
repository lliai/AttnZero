...
from autoatten.attentions import AutoAttention_pvt
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_patches, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        if attn_type == 'A':
            self.attn = AutoAttention_pvt(atten_cfg, num_patches,
                dim, num_heads=num_heads, qkv_bias=qkv_bias,
                attn_drop_rate=attn_drop, proj_drop_rate=drop, sr_ratio=sr_ratio)
        else:
            self.attn = Attention(
                dim,
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        for i in range(num_stages):
            ...
            block = nn.ModuleList([Block(
                ...
                norm_layer=norm_layer, sr_ratio=sr_ratios[i] if attn_type[i] == 'B' else int(la_sr_ratios[i]),
