...
from autoatten.attentions import AutoAttention_swin
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        if attn_type == 'A':
            self.attn = AutoAttention_swin(atten_cfg, window_size=to_2tuple(self.window_size),  dim=dim,
                                           num_heads=num_heads, qkv_bias=qkv_bias, attn_drop_rate=attn_drop, proj_drop_rate=drop)
        else:
...
class AutoAttenSwinTransformer(nn.Module):
    r""" Swin Transformer
