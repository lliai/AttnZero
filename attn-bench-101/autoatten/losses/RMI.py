...
def gram_matrix(x):  #np和tensor都可以用这个方法
...
class RMIloss(nn.Module):
    ...
    def forward(self, features_1, features_2):
        ...
        for i in range(len(features_1)):
            s.append(
                tensor_cka(
                    tensor_gram_matrix(features_1[i].view(self.datasize, -1)),
                    tensor_gram_matrix(features_2[i].view(self.datasize, -1))))
