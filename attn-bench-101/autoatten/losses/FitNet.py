...
def batch_kl(preds_S, preds_T):
    ...
    log_t = softmax_pred_T * logsoftmax(preds_T.view(-1, C * W * H) / 1.0)
    log_s = softmax_pred_T * logsoftmax(preds_S.view(-1, C * W * H) / 1.0)
    loss = torch.sum(log_t - log_s) * (1.0**2)
    return loss / (C * N)
...
def local_att_channel_l2(f_s, f_t):
    return channel_l2(
        trans_att(trans_local(f_s, 4)), trans_att(trans_local(f_s, f_t)))
...
def ms_l2(f_s, f_t):
    return (F.mse_loss(f_s, f_t) +
            F.mse_loss(trans_ms(f_s, 2), trans_ms(f_t, 2)) +
            F.mse_loss(trans_ms(f_s, 4), trans_ms(f_t, 4))) / 3
