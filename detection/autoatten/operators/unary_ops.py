...
UNARY_KEYS = ('no_op', 'transpose', 'softmax', 'element_wise_log', 'element_wise_abslog', 'element_wise_abs', 'normalize',
               'softsign', 'element_wise_invert',  'sigmoid',
              'logsoftmax', 'element_wise_sqrt', 'element_wise_revert',
              'min_max_normalize',   'element_wise_sign',  'scale', 'elu', 'axis_sum1', 'unsqz1',
              'element_wise_relu', 'l2_norm', 'element_wise_exp', 'unsqz2', 'axis_sum2', 'repeat', 'l2norm_last',
              'sigmoid_1_reverse', 'strange_op'
              )
...
def sample_unary_key_by_prob(probability=None):
    if probability is None:
        ...
        probability = [0.8]+ (len(UNARY_KEYS) - 1)*[0.05]
...
def transpose(A: ALLTYPE) -> ALLTYPE:
    return A.transpose(-1, -2)



def scale(A: ALLTYPE) -> ALLTYPE:
    b, num_heads, L, head_dim = A.shape
    scale = head_dim** -0.5
    return A*scale



def elu(A: ALLTYPE) -> ALLTYPE:
    return F.elu(A) + 1
...
def axis_sum1(A: ALLTYPE) -> ALLTYPE:
    return A.sum(axis=2)

def axis_sum2(A: ALLTYPE) -> ALLTYPE:
    return A.sum(axis=1)

def unsqz1(A: ALLTYPE) -> ALLTYPE:
    return A.unsqueeze(-1)

def unsqz2(A: ALLTYPE) -> ALLTYPE:
    return A.unsqueeze(0)

def l2norm(A: ALLTYPE) -> ALLTYPE:
    return F.normalize(A, dim = -2, p = 2)



def l2norm_last(A: ALLTYPE) -> ALLTYPE:
    return F.normalize(A, dim = -1, p = 2)



def strange_op(A: ALLTYPE) -> ALLTYPE:
    A /= A.sum(dim=-1).unsqueeze(-1)
    return A
...
def repeat(A: ALLTYPE) -> ALLTYPE:
    return 1/ (A.repeat(1, 1, 192) + 1e-8)
...
def softsign(A: ALLTYPE) -> ALLTYPE:
    return F.softsign(A)
...
def sigmoid_1_reverse(A: ALLTYPE) -> ALLTYPE:
    return 1. - torch.sigmoid(A)


def logsigmoid(A: ALLTYPE) -> ALLTYPE:
...
def unary_operation(A, idx=None):
    ...
    unaries = {
        ...
        'axis_sum2': axis_sum2,
        'repeat': repeat,
        'l2norm_last': l2norm_last,
        'sigmoid_1_reverse':sigmoid_1_reverse,
        'strange_op': strange_op

    }
    # print('idx:', idx)
    # print(UNARY_KEYS[idx])
