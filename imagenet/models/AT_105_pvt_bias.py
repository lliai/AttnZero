...
class AutoAttention_Bias(nn.Module):
    def __init__(self, dim, num_patches, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,
        ...
        self.na_bias = nn.Parameter(torch.zeros(num_heads, 64, 7, 7))
        self.ha_bias = nn.Parameter(torch.zeros(1, num_heads, window_size[0], 1, 64))
        self.wa_bias = nn.Parameter(torch.zeros(1, num_heads, 1, window_size[1], 64))
    ...
    def forward_diag(self, x, agent_bias):
        ...
        # return res
        #
        # q.size: torch.Size([128, 8, 49, 64])
        # A3 before B1: torch.Size([128, 8, 49, 64])
        # A4 before B1: torch.Size([128, 8, 49, 64])
        # B1: torch.Size([128, 8, 64, 64])
        # A2 before B2: torch.Size([128, 8, 49, 64])
        # B1 before B2: torch.Size([128, 8, 64, 64])
        # B2: torch.Size([128, 8, 49, 64])
        # A1 before B3: torch.Size([128, 8, 49, 64])
        # B2 before B3: torch.Size([128, 8, 49, 64])
        # B3: torch.Size([128, 8, 49, 64])

        q, k, v = [x[i] for i in range(3)]
        binary_ops = self.atten_cfg['binary_op']    # 6,  3,  2
        unary_ops = self.atten_cfg['unary_op']
        A1, A2, A3, A4 = q.clone(), q.clone(), k.clone(), v.clone()
        ...
        # print('B2 before add:', binary_operation(A2, B1, binary_ops[1]).size())   # torch.Size([128, 5, 196, 64])
        # print('agent bias before add:', agent_bias.size())   # torch.Size([128, 5, 196, 64])
        B2 = binary_operation(A2, B1, binary_ops[1]) + agent_bias
    ...
    def forward(self, x, H, W):
        ...
        agent_bias1 = agent_bias1.reshape(1, num_heads, 64, -1).permute(0, 1, 3, 2).repeat(b, 1, 1, 1)
        agent_bias2 = (self.ha_bias + self.wa_bias).reshape(1, num_heads, -1, 64).repeat(b, 1, 1, 1)
        agent_bias = agent_bias1 + agent_bias2


        x = self.forward_diag([q, k, v], agent_bias)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_patches, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 attn_type='A', atten_cfg=None):
        super().__init__()
        self.norm1 = norm_layer(dim)
        assert attn_type in ['A', 'B']
        if attn_type == 'A':
            self.attn = AutoAttention_Bias(
                dim, num_patches,
...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 ...
                 attn_type='AAAA', atten_cfg=None, **kwargs):
        super().__init__()
        ...
        for i in range(num_stages):
            ...
            block = nn.ModuleList([Block(
                ...
                norm_layer=norm_layer, sr_ratio=sr_ratios[i] if attn_type[i] == 'B' else int(agent_sr_ratios[i]),
                 attn_type=attn_type[i], atten_cfg=atten_cfg)
...
def AutoAtten_Bias_pvt_tiny(pretrained=False, **kwargs):
...
# def AutoAtten_pvt_small(pretrained=False, **kwargs):
#     model = PyramidVisionTransformer(
#         patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
...
#
#
# def AutoAtten_pvt_medium(pretrained=False, **kwargs):
#     model = PyramidVisionTransformer(
#         patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
...
#
#
# def AutoAtten_pvt_large(pretrained=False, **kwargs):
#     model = PyramidVisionTransformer(
#         patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
